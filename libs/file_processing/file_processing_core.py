    ...
    def chunk_not_exists_case(
        ...
        if data_type in SURVEY_DATA_FILES:
            ...
            survey_id = Survey.objects.filter(
                object_id=self.survey_id_dict[survey_id_hash]).values_list("pk", flat=True).get()
        else:
            survey_id = None
        ...
        self.upload_these.append((chunk_params, chunk_path, compress(new_contents), study_object_id))
...
def upload_binified_data(binified_data, error_handler, survey_id_dict):
    ...
    uploads = Uploader(binified_data, error_handler, survey_id_dict)
    uploads.iterate()
